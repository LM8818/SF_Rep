...
class NLPService:
    ...
    def _load_sample_data(self) -> pd.DataFrame:
        """Загружает примеры данных для обучения"""
        # Создаем примеры данных
        data = {
            'text': [
                'Хочу узнать про ипотеку',
                'У меня проблема с картой',
                'Интересует вклад под проценты',
                'Нужен кредит на ремонт',
                'Хочу оформить страховку'
            ],
            'theme': ['ипотека', 'жалоба', 'вклад', 'кредит', 'страхование']
        }
    ...
    def _train_model(self, df: pd.DataFrame):
        ...
        joblib.dump(self.model, 'models/latest_model.joblib')
    ...
    async def analyze_conversation(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.model and self.vectorizer:
                ...
                theme_prediction = "информация"
                theme_probability = 0.5
